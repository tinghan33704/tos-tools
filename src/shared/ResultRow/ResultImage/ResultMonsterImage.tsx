...
export const ResultMonsterImage = React.memo<IResultMonsterImageProps>(
    (props) => {
        const {
            data,
            noImagePopover,
        ...
        const renderMonsterName = useCallback(() => {
            /***** EASTER EGG *****/
            /* Attribute icon change for Yugioh */
            const specialAttrIcon = monsterTag?.includes("遊戲王 怪獸之決鬥")
                ? `${attrZhToEn?.[attribute]}_yugioh`
                : null
            ...
            const attrPath = specialAttrIcon
                ? `other/icon_${specialAttrIcon}`
                : `icon/icon_${attrStr}`
            ...
            return (
                <Row
                    className={`result-info-header result-info-header-${attrStr}`}
                >
                    ...
                    <Row className='monster-skill'>
                        <Row className='monster-skill-name-row'>
                            ...
                            <Col
                                xs={3}
                                sm={3}
                                className={"monster-skill-charge"}
                            >
                                {`${skill?.charge || ""} `}
                                {"reduce" in skill
                                    ? `${skill?.num} → ${
        ...
        const renderImage = useCallback(() => {
            ...
            return (
                <>
                    ...
                    />
                    {(skills[skillIndexes?.[0] || skillIndex]?.type ===
                        "combine" ||
